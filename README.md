...
| Skill | 说明 | 模型 |
|-------|------|------|
| **agnes-image-gen** | 文生图 / 图生图 | `agnes-image-2.1-flash` / `agnes-image-2.0-flash` |
| **agnes-video-gen** | 文生视频 / 图生视频 / 多图视频 / 关键帧动画 | `agnes-video-v2.0` |
...
将 `agnes-image-gen` 和/或 `agnes-video-gen` 文件夹复制到 WorkBuddy 的 skills 目录：
...
- `"帮我生成一张..."` → 自动使用 agnes-image-gen
- `"用这个图片生成视频..."` → 自动使用 agnes-video-gen
