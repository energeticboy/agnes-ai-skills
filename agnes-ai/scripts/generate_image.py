...
# Windows console encoding workaround: replace emoji with text labels
_ICONS = {"art": "[Art]", "download": "[DL]", "save": "[OK]", "error": "[ERR]",
           "image": "[Img2Img]"}
...
def generate_text_to_image(
    ...
    _safe_print(f"[Art] Text-to-Image generating...")
...
def generate_image_to_image(
    ...
    _safe_print(f"[Img2Img] Image-to-Image generating...")
    _safe_print(f"         Prompt:     {prompt}")
...
Examples:
  ...
  python generate_image.py --api-key KEY --prompt "水彩风格" --image https://example.com/photo.png
    ...
    parser.add_argument(
        "--api-key",
        required=False,
        help="Agnes AI API key (or set AGNES_API_KEY env var)",
    )
    parser.add_argument(
        "--prompt",
        required=True,
        help="Image generation prompt (supports Chinese and English)",
    )
    parser.add_argument(
        "--size",
        default="1024x1024",
        help="Output image size (default: 1024x1024). Options: 1024x1024, 1024x768, 768x1024, 512x512",
    )
    parser.add_argument(
        "--image",
        action="append",
        default=[],
        help="Reference image URL(s) for img2img. Repeat for multiple images: --image URL1 --image URL2",
    )
    parser.add_argument(
        "--output",
        "-o",
